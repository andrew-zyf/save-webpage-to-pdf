...
- 自动抽取 **标题 / 作者 / 正文 / 评论容器**，无需确认直接打印
- 图片不跨页、说明（caption）与图同页、超大图等比缩到单页
- 自动隐藏小漂浮 fixed、广告占位文字、收听/打印/分享类按钮
- 在标题后插入 `BY: <作者>` + `SOURCE: <url>` 横幅
- 内置站点规则（WSJ / Foreign Affairs / New Yorker / Economist / CSIS / CNN）抽取标题/作者/正文/评论容器，并按段落文本评分挑选正文，避免选到「最新新闻 / 相关阅读」列表
